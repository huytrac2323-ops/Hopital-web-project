...
function AppointmentList() {
    ...
    useEffect(() => {
        ...
        // Fetch doctors
        fetch(`${process.env.REACT_APP_API_URL}/api/doctors`, { headers: getAuthHeaders(), signal })
            .then(response => {
    ...
    const fetchAppointments = (signal) => {
        fetch(`${process.env.REACT_APP_API_URL}/api/appointments`, { headers: getAuthHeaders(), signal })
            .then(response => {
                if (response.status === 401) {
                    navigate('/login');
                    return Promise.reject(new Error('Unauthorized'));
